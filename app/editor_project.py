...
EDITOR_PROJECT_FORMAT = "acorn-file-forge-editor-project-1"
REGION_KINDS = {"code", "text", "bytes", "words", "addresses", "bitmap"}


def editor_project_key(path: str, slot: int | None, side: int | None) -> str:
    return f"{slot if slot is not None else '-'}|{side if side is not None else '-'}|{path}"
