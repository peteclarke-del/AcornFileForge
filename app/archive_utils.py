    ...
    with _open_zip(upload) as archive:
        ...
        if len(members) != 1:
            ...
            raise DiskError(
                ...
                f"({names}{suffix}). Insert it into an MMB to import all SSD/DSD "
...
def open_disk_image_upload(
    ...
    """Open one disk image and an optional matching DSC from a ZIP upload."""
    if not is_zip_name(upload.filename):
        yield ArchiveImage(upload.filename, upload.stream, [upload.filename]), None
        return
    with _open_zip(upload) as archive:
        members = _supported_members(archive, image_extensions | {".dsc"})
        images = [
            member
            for member in members
            if Path(member.filename).suffix.lower() != ".dsc"
        ]
        if len(images) != 1:
            raise DiskError(
                f"{upload.filename} must contain exactly one supported disk or "
                f"tape image; {len(images)} were found."
            )
        image_info = images[0]
