...
class CheckpointStore:
    ...
    def _state(session: ImageSession) -> dict:
        return {
            ...
            "dirty": session.dirty,
            "slotSourceNames": {
                str(slot): name for slot, name in session.slot_source_names.items()
            },
            "adfsSourceNames": dict(session.adfs_source_names),
    ...
    def restore(self, session: ImageSession, checkpoint_id: str) -> dict:
        ...
        session.slot_source_names = {
            int(slot): str(name)
            for slot, name in (state.get("slotSourceNames") or {}).items()
        }
        session.adfs_source_names = {
            str(path): str(name)
            for path, name in (state.get("adfsSourceNames") or {}).items()
        }
