  ...
  function create({ esc, normalisePage, trapFocus }) {
    ...
    function confirmPageOverride(defaultPage, chosenPage, subjects = []) {
      if (Array.isArray(defaultPage)) {
        ...
        return showPageWarning({ heading: `Use ${overrides.length} changed PAGE ${overrides.length === 1 ? "value" : "values"}?`, body: "<p>These values differ from the launchers in the actual disk images.</p>", list, confirmLabel: "Yes, use changed values" });
      }
      ...
      return showPageWarning({
        heading: "Use a different PAGE value?",
