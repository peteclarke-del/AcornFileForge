window.AcornFileVisuals = (() => {
  ...
  const archiveNamePattern = /\.(?:uef|zip|tar|tgz|tbz2?|txz|gz|gzip|bz2|xz)$/i;
  const scriptNamePattern = /^(?:!boot|boot|startup|start|loader|menu)(?:\.|$)/i;

  function fileKindKey(pane, name) {
    return [pane.slot ?? "image", pane.side ?? "side", pane.path, pane.archivePath || "", pane.archiveMember || "", name]
      .join("|")
      .toLocaleLowerCase();
  }
  ...
  function entryIcon(pane, entry, entryType, isArchiveFile, isVirtual) {
    ...
    else if (isArchiveFile || cached === "container" || archiveNamePattern.test(name)) [kind, label] = ["archive", "Archive or tape container"];
    else if (cached === "basic" || filetype === "FFB" || /\.(?:bas|basic)$/i.test(name)) [kind, label] = ["basic", "BBC BASIC program"];
    else if (cached === "script" || filetype === "FEB" || scriptNamePattern.test(name)) [kind, label] = ["script", "Command script"];
