window.AcornUI = (() => {
  const OWNER_STORAGE_KEY = "acorn-file-forge-session-owner";
  ...
  async function api(url, options = {}) {
    ...
    if (owner) headers.set("X-Acorn-Session-Owner", owner);
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      try {
        ...
        if (error?.name === "AbortError" || attempt >= retries) {
          ...
          const interrupted = new Error(
            `Connection to Acorn File Forge was interrupted after ${attempt + 1} attempts. The image remains available; try the operation again.`
          );
      ...
      rememberOwner(response.headers.get("X-Acorn-Session-Owner"));
  ...
  function uploadApi(
    ...
    return new Promise((resolve, reject) => {
      ...
      const owner = storedOwner();
      if (owner) request.setRequestHeader("X-Acorn-Session-Owner", owner);
      ...
      request.addEventListener("load", () => {
        rememberOwner(request.getResponseHeader("X-Acorn-Session-Owner"));
      ...
      request.addEventListener("error", () => {
        reject(new Error(
          "The upload connection failed before Acorn File Forge received the image."
        ));
      });
  ...
  function showModalError(error) {
    ...
    const details = [
      ...
      ...(Array.isArray(skipped) && skipped.length
        ? [{
            label: "Items skipped",
            value: `${skipped.length} ${skipped.length === 1 ? "item was" : "items were"} not copied to ADFS`
          }]
        : []),
