        ...
        <div class="help-content">
          ...
          <section id="help-desktop">
            ...
            <div class="help-task">
              ...
              <ol>
                ...
                <li>Review the selection before it opens. The active Workbench profile supplies the initial ADFS target, which can be changed for this operation. Multiple ROM files may be opened separately or treated as one linear or byte-interleaved physical component set.</li>
              </ol>
            </div>
            <ul>
            ...
            <div class="help-note"><strong>Icon opens no window:</strong> pull the current code and rerun <code>tools/install-linux-desktop.sh</code>. The launcher detects Ubuntu systems that deny WebKitGTK's Bubblewrap user namespace and enables the compatibility fallback only there. Set <code>ACORN_FILE_FORGE_DISABLE_WEBKIT_SANDBOX=0</code> to require sandboxing or <code>1</code> for diagnostic fallback. Run <code>~/.local/bin/acorn-file-forge</code> in a terminal if startup diagnostics are still needed.</div>
            <div class="help-warning"><strong>Updating:</strong> choose <strong>About Acorn File Forge</strong> from the <strong>Help</strong> menu and press <strong>Check for Application Updates</strong>. A release package then offers <strong>Update to</strong> the new version, downloads the package for the same system, checks it against the release's <code>SHA256SUMS</code>, installs it with APT after the system asks for your password, and offers to restart. Nothing is installed while a floppy disk is being read or written. A checkout is sent to the release page instead: pull the new source and rerun the checkout installer when Python dependencies change. Working sessions remain under <code>~/.local/share/acorn-file-forge</code>, or the configured XDG data directory.</div>
          </section>
          ...
          <section id="help-online">
            ...
            <h4>Sources, availability and safety</h4><ul>
              ...
              <li>Every Game Going maps BBC B, B+, Master 128/Compact, Electron and Archimedes A3000 machine IDs from provider settings. Each matching item page is checked for actual downloadable media before it is displayed. Its shared BBC/Electron records are classified for both families. Continuation checks expose the whole matching index without opening thousands of remote pages at once.</li>
          ...
          <section id="help-tapes">
            ...
            <div class="help-task">
              ...
              <ol>
                <li>Open the UEF in any pane. The catalogue is reconstructed from physical tape blocks rather than treated as a disk directory.</li>
          ...
          <section id="help-analysis">
            ...
            <figure><img src="/help/health-dashboard.png" alt="Image health dashboard with an expanded failed MMB menu record"><figcaption>Each failed menu check includes actionable evidence. Expand it to see the menu location, target disk or slot, launch command, PAGE and exact problem.</figcaption></figure>
            ...
            <div class="help-task"><h4>Inspect a file or loader</h4><ol>
              ...
              <li>Open one tokenised BBC BASIC or machine-code file and choose <strong>Tools → Find cheat candidates</strong> inside its editor for a read-only report. On a wide editor the report docks on the right at full listing height and scrolls independently; narrow windows place it below the code. Drag the separator, or focus it and use the arrow keys, to resize the two sections. Select a candidate to centre and highlight its BASIC line or decoded address. BASIC requires corroboration between semantic state, plausible initialisation, updates, terminal tests and gameplay outcomes. Machine code joins initialisation, access to the same storage, updates, forward terminal branches and saved labels across 6502, 65C02, 65816, ARM and 68000. Reachable unlabelled state changes with a forward decision are retained as Possible; speculative instructions decoded from embedded data are excluded. Unexplained writes, opaque countdowns, backward decrement loops, hardware registers and likely copy, clear, scan or delay counters are suppressed. Loader commands and packed or runtime-generated payloads are identified explicitly when static analysis cannot reach the final game code. Trainer-style NOP, RTS, JMP and BIT writes remain separately identified with a warning. Filter by likely purpose or confidence. Online title evidence and configured specialist searches are optional and never prove that similarly named bytes match. For a selected machine-code result with an exact offset, <strong>Prepare guarded patch</strong> records the complete source hash, guarded bytes, hardware profile, two tester-supplied emulator observations, rationale, author and rollback instructions. Apply verifies the hash and bytes again and creates an automatic checkpoint. The host-private library matches exact files, never titles. Automatic watchpoint correlation is not claimed.</li>
              ...
              <li><strong>Tools → Run… / Debug…</strong> appears in every pane whose media can be attached to the configured machine. Standalone DFS, ADFS and tape images mount directly. At an MMB index, select exactly one formatted slot to run its temporary SSD, or use the separate whole-MMB commands with an Electron MMFS profile. Whole-MMB launch builds a private FAT32 card containing root <code>BEEB.MMB</code>, attaches it through the bundled Elkulator Pi1MHz raw-SD adapter and loads the selected paged or unpaged MMFS ROM. The working MMB remains isolated from emulator writes. BBC and Master emulators explain that they have no equivalent whole-card adapter.</li>
              <li>In an installed MMB menu preview, choose <strong>Capture actual menu</strong> to boot that private card on a bounded display. The app records a settled screen, sends one navigation key, records the changed screen and ties both hashes to the exact MMB revision. Repeat the capture to prove stable screen hashes. Image Health reports repeatable display and input evidence separately from its per-entry launcher and PAGE checks.</li>
            ...
            <div class="help-task"><h4>Audit a collection</h4><ol>
              ...
              <li>Choose <strong>Search</strong> in the application header to search every distinct open image with one query. It matches filenames, catalogue filetype and access metadata, load and execution addresses, bounded BASIC or script text, useful printable strings in binary files and raw ROM banks, recognised MMB and ADFS menu titles, publishers, launch actions and PAGE values, and ROM Workbench symbols, regions, notes and comments. An 8 to 64 digit SHA-256 prefix finds exact file content and displays the complete digest. Results identify their pane, image, MMB slot, path or ROM bank. Selecting one restores and raises its pane, navigates to the containing location and opens the file or relevant ROM Workbench address.</li>
            ...
            <figure><img src="/help/private-collection.png" alt="Private collection catalogue showing an indexed DFS image, location and target machines"><figcaption>The host-private catalogue remains searchable after an image is closed. Locations are descriptive and the database stores manifests rather than image bytes.</figcaption></figure>
            ...
            <div class="help-task"><h4>Profiles, recipes and projects</h4><ol>
              ...
              <li>In the same screen, choose an open image and select <strong>Export workflow bundle</strong> for a deterministic rebuild package. It records the earliest retained pre-change image and DSC hashes, a guarded patch for every subsequent filesystem change, the active hardware and accepted compatibility decisions, and the exact expected output hashes. The original image bytes are not included. Extract the ZIP and follow its README; <code>recipe-run</code> refuses a changed base, descriptor, patch or rebuilt result. Edited legacy sessions without a retained base, UEF tapes and HFE track containers are rejected rather than represented as safely reproducible.</li>
            </ol></div>
          ...
          <section id="help-limits">
            ...
            <ul>
              ...
              <li>UEF members are editable only under the tape project's same-length structural proof. Convert or copy reconstructed files into writable disk media for unrestricted filesystem editing.</li>
            ...
            <dl>
              ...
              <dt>Invalid filename</dt><dd>Use the prompted replacement. DFS leaf names allow seven Latin-1 characters, MMB disk titles allow twelve, ROMFS leaves allow ten, normal ADFS directories allow ten and FileCore Big directories allow up to 255. Leading or trailing whitespace, path syntax and control characters are rejected. The compatibility review normalises and truncates before writing, then checks case-insensitive clashes within each destination directory.</dd>
          ...
          <section id="help-project">
            ...
            <div class="help-note"><strong>Confirm the running build:</strong> close this handbook, then choose <strong>Help → About Acorn File Forge</strong>. The About dialog reports the version returned by the current server, the web or Linux desktop edition, filesystem engine, licence and project links. Its <strong>Check for Application Updates</strong> button asks GitHub for the latest release only when pressed.</div>
            <div class="help-task"><h4>Choose the detailed reference</h4><ul>
              ...
              <li><a href="https://github.com/peteclarke-del/AcornFileForge/blob/main/docs/CLI-GUIDE.md" target="_blank" rel="noopener noreferrer">Headless CLI and deterministic recipes</a>: Docker invocation, stable JSON results, dry-runs, source identity checks and repeatable image builds.</li>
            </ul></div>
