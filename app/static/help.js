        ...
        <div class="help-content">
          ...
          <section id="help-analysis">
            ...
            <div class="help-task"><h4>Audit a collection</h4><ol>
              <li>Choose <strong>Search</strong> in the application header to search every distinct open image with one query. Filename and bounded readable-content matches identify their pane, image, MMB slot and path. Selecting one restores and raises its pane, navigates to the containing location and opens the file. Raw ROM images retain their dedicated ROM Workbench and Hex searches.</li>
              ...
              <li><strong>Compare with open image</strong> matches two manifests by filesystem path, MMB slot, DFS side or ROM bank. It separates added and removed objects, changed bytes and metadata-only edits. Export JSON retains the deterministic logical fingerprints and full itemised change report. Different filesystem families can be compared as inventories but are marked unsuitable for direct patching.</li>
              <li>For matching filesystem families, DFS side layouts and ROM bank sizes, <strong>Download patch</strong> creates an <code>.affpatch.zip</code> containing the reviewed operation plan and only changed payloads. Payloads are checksummed and streamed into the archive, keeping large FileCore batches out of application memory. <strong>Analyse → Apply guarded patch</strong> first performs a read-only preflight against the exact base fingerprint and verifies every payload. It shows the base and candidate names, change totals and an itemised operation preview; Apply remains disabled until verification succeeds. Applying creates an automatic checkpoint, repeats validation before writing and checks the complete candidate fingerprint afterwards. Stale, corrupt and wrong-format patches are rejected and failed applications roll back.</li>
