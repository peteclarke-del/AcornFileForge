...
def analyse_basic(source: str) -> list[dict]:
    """Find gameplay-state and direct-memory evidence in a BBC BASIC listing."""
    findings: list[dict] = []
    ...
    if len(reachable) <= max(2, len(rows) // 100):
        ...
        diagnostics.append({
            ...
            "detail": (
                ...
                "so embedded graphics and packed data were not mistaken for 6502 instructions."
            ),
        })
    return diagnostics
...
def cheat_report(*, path: str, kind: str, findings: list[dict], title: str = "", machine: str = "", matches: list[dict] | None = None, diagnostics: list[dict] | None = None) -> dict:
    ...
    return {
        ...
        "referenceSearches": reference_searches(title or path.rsplit(".", 1)[-1], machine),
        "readOnly": True,
