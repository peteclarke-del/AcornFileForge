"""Filename and online metadata discovery for imported Acorn software."""
...
def _lookup_bbc_archive(query: str, timeout: float) -> list[dict]:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "AcornFileForge/1.0 metadata lookup"}
    )
    with urllib.request.urlopen(request, timeout=timeout) as response:
        source = response.read(1_000_000).decode("utf-8", "replace")
    links: list[tuple[str, str]] = []
    for match in re.finditer(
        r'(?is)href=["\'](?:https?://[^"\']+)?/?game\.php\?id=(\d+)[^"\']*["\'][^>]*>(.*?)</a>',
        source,
    ):
...
def lookup_online(query: str, timeout: float = 6.0) -> list[dict]:
    """Search specialist Acorn records, then broader archive catalogues."""
