...
def _open_snapshot(
    ...
    descriptor_name = str(state.get("descriptorName") or "image.dsc")
...
def build_workflow_recipe_bundle(
    ...
    if session.kind == "tape":
        raise DiskError("Writable UEF workflow recipes are not available yet.")
        ...
        with tempfile.TemporaryDirectory(dir=service.work_dir, prefix="workflow-recipe-") as work:
            ...
            if base_descriptor and sources["image"].get("descriptor"):
                sources["image"]["descriptor"]["name"] = str(
                    state.get("descriptorName") or session.descriptor_name or "image.dsc"
                )
...
{f"- DSC filename: `{descriptor['name']}`\n- DSC size: `{descriptor['size']}` bytes\n- DSC SHA-256: `{descriptor['sha256']}`" if descriptor else ""}

## Rebuild

Extract this ZIP, then run from an Acorn File Forge checkout:
