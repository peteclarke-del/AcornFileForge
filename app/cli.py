"""Supported automation interface for Acorn File Forge."""
...
def _image_arguments(parser: argparse.ArgumentParser, name: str = "image") -> None:
    ...
    parser.add_argument("--descriptor", type=Path, help="Matching BeebSCSI DSC descriptor")
...
def build_parser() -> argparse.ArgumentParser:
    parser = JsonArgumentParser(prog="acorn-file-forge")
    ...
    create.add_argument("--platform", default="bbc")
    ...
    validate.add_argument("--slot", type=int)
    ...
    import_file.add_argument("--slot", type=int)
    import_file.add_argument("--side", type=int)
    import_file.add_argument("--load")
    import_file.add_argument("--execute")
    import_file.add_argument("--filetype")
    _output_arguments(import_file)

    convert = sub.add_parser("convert", help="Convert a UEF tape to SSD or DSD")
    _image_arguments(convert)
    convert.add_argument("--format", choices=("ssd", "dsd"), required=True)
    _output_arguments(convert)

    compact = sub.add_parser("compact", help="Compact a writable filesystem")
    _image_arguments(compact)
    compact.add_argument("--slot", type=int)
    compact.add_argument("--order")
    _output_arguments(compact)

    menu = sub.add_parser("menu-create", help="Create a supported MMB or ADFS menu")
    _image_arguments(menu)
    menu.add_argument("--menu-type", choices=("universal", "spi-game-menu", "electron-magazine", "acorn-user"), default="universal")
    menu.add_argument("--slot", type=int, default=0)
    menu.add_argument("--page", default="current")
    menu.add_argument("--root", default="$")
    menu.add_argument("--entries", type=Path, help="ADFS menu entries as a JSON array")
    _output_arguments(menu)
...
def _validate(args, _progress) -> dict:
    with open_image(args.image, **_open_kwargs(args)) as (service, session):
        return {"image": service.summary(session), "message": service.validate(session, args.slot)}
...
def _convert(args, progress) -> dict:
    with open_image(args.image, **_open_kwargs(args)) as (service, session):
        ...
        action = {"action": "convert-uef", "format": args.format, **_recorded_open_context(args)}
        converted, files = service.convert_uef(session, args.format)
        if args.dry_run:
...
def _compact(args, progress) -> dict:
    with open_image(args.image, **_open_kwargs(args)) as (service, session):
        identity = source_identity(args.image, descriptor=args.descriptor, service=service, session=session)
        action = {"action": "compact", "slot": args.slot, "order": args.order, **_recorded_open_context(args)}
        service.compact(session, args.slot, args.order)
        if args.dry_run:
            return {"image": identity, "action": action, "output": str(args.output), "validated": True}
        outputs = save_image(service, session, args.output, force=args.force, progress=progress)
        _record_recipe(args, {"image": identity}, [action], outputs)
    ...
    with open_image(args.image, **_open_kwargs(args)) as (service, session):
        ...
        if args.entries:
            try:
                ...
                raise DiskError(f"The ADFS menu entries are not readable JSON: {exc}") from exc
            if not isinstance(entries, list):
                raise DiskError("ADFS menu entries must be a JSON array.")
        action = {
            "action": "menu-create",
            **_recorded_open_context(args),
        ...
        if session.kind == "mmb":
            installed = install_menu(service, session, menu_type=args.menu_type, slot=args.slot, page=args.page)
        elif session.kind == "adfs":
            if not entries:
                raise DiskError("ADFS menu creation requires a non-empty --entries JSON array.")
            installed = create_directory_menu(service, session, root=args.root, entries=entries)
        else:
            raise DiskError("Menus can be created only in MMB or ADFS images.")
        if args.dry_run:
            return {"image": identity, "action": action, "output": str(args.output), "menu": installed, "validated": True}
        outputs = save_image(service, session, args.output, force=args.force, progress=progress)
        _record_recipe(args, {"image": identity}, [action], outputs)
        return {"menu": installed, "files": outputs}
...
def _patch_create(args, progress) -> dict:
    if args.dry_run:
        with tempfile.TemporaryDirectory(prefix="acorn-file-forge-patch-dry-run-") as work:
    ...
    with open_image(
        ...
        for action in actions:
            ...
            if kind == "import-file":
                ...
                install_menu(service, session, menu_type=action["menuType"], slot=int(action.get("slot", 0)), page=action.get("page", "current"))
            elif kind == "menu-create" and session.kind == "adfs":
                create_directory_menu(service, session, root=action.get("root", "$"), entries=action.get("entries") or [])
            elif kind == "save":
                continue
