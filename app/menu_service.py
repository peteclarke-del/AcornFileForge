...
def _looks_like_adfs_holder(entries: list[dict]) -> bool:
...
def discover_adfs_menu_paths(
    ...
    for row in root_listing["entries"]:
        ...
        listing = service.list_directory(session, path, None)
        if _looks_like_adfs_holder(listing["entries"]):
            holders.append(path)
            paths.extend(
                f"{path}.{child['name']}"
...
def find_menu_slot(service: DiskService, session: ImageSession) -> int | None:
    if session.menu_scanned:
        return (
            session.menu_slot
            if getattr(session, "menu_type", None) != "mmc-desktop"
            else None
        )
    fast_finder = getattr(service, "find_mmb_slot_with_catalogue_files", None)
    if fast_finder is not None and session.kind == "mmb":
        session.menu_slot = fast_finder(session, MENU_FILES)
        session.menu_type = "universal" if session.menu_slot is not None else None
        if session.menu_slot is None:
            session.menu_slot = fast_finder(session, UNIVERSAL_4R_FILES)
            session.menu_type = (
                "universal-4r" if session.menu_slot is not None else None
            )
        if session.menu_slot is None:
            session.menu_slot = fast_finder(session, SPI_GAME_MENU_FILES)
            session.menu_type = (
                "spi-game-menu" if session.menu_slot is not None else None
            )
    ...
    for entry in service.list_slots(session):
        if (
            entry["formatted"]
            and not is_mmb_menu_backup_title(entry.get("name"))
            and is_universal_menu(service, session, entry["slot"])
        ):
...
def installed_mmb_menu(
    ...
    if menus:
        selected = menus[0]
        return int(selected["slot"]), str(selected["type"])
    return None, None


def installed_mmb_menus(
    service: DiskService,
    session: ImageSession,
) -> list[dict]:
    ...
    bulk_finder = getattr(
        service,
        "find_mmb_slots_with_catalogue_files",
        None,
    )
    ...
    if bulk_finder is not None:
        ...
        candidates = (
            (menu_type, found.get(menu_type))
            for menu_type, _files in signatures
        )
    elif finder is not None:
        candidates = (
            (menu_type, finder(session, files))
            for menu_type, files in signatures
        )
    else:
        slot = find_menu_slot(service, session)
        return (
            [{"slot": slot, "type": session.menu_type or "universal"}]
            if slot is not None
            else []
        )
    menus = []
    ...
    for menu_type, slot in candidates:
        if slot is not None and slot not in seen_slots:
            ...
            seen_slots.add(slot)
    if menus:
        session.menu_slot = int(menus[0]["slot"])
        session.menu_type = str(menus[0]["type"])
    else:
...
def _menu_type_from_ssd(data: bytes) -> str | None:
