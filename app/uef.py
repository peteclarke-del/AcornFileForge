...
def _blocks(stream: bytes) -> list[_Block]:
    ...
    while position < len(stream):
        ...
        blocks.append(_Block(name, load, execute, number, payload, bool(flags & 0x80)))
        position = end
    return blocks
...
def parse_uef(data: bytes) -> UEFContents:
    ...
    for group in groups:
        ...
        complete = last is not None and all(number in group["parts"] for number in range(last + 1))
        data_parts = [group["parts"][number] for number in range(expected_last + 1) if number in group["parts"]]
