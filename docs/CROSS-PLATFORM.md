...
Acorn File Forge is one application with several shells. The Flask service, the
...
| --- | --- | --- |
| Linux | `$XDG_DATA_HOME/acorn-file-forge`, `$XDG_CONFIG_HOME/acorn-file-forge` | `$XDG_DATA_HOME/acorn-file-forge/work` |
| macOS | `~/Library/Application Support/AcornFileForge` | the same, under `work` |
| Windows | `%APPDATA%\AcornFileForge` | `%LOCALAPPDATA%\AcornFileForge\work` |
...
install byte-identical application trees under `/opt/acorn-file-forge`. Only the
metadata, dependency names and scriptlets differ. A fault reproduced on one
distribution therefore reproduces on the other.

Set `ACORN_RPM_RELEASE` to control the release field, for example
`ACORN_RPM_RELEASE=2.fc41`.
...
- **`disc`**, the Oaknut filesystem engine, installs from PyPI with the pinned
...
- **`hxcfe`**, the HxCFloppyEmulator converter, is a C program built from
  ...
  everything else, including DFS, ADFS, MMB, UEF and ROM work, runs without it.
