...
**About Acorn File Forge**, in the **Help** menu, has a **Check for
...
3. The system asks for your password, and `pkexec apt-get install` installs the
   ...
   collection are kept.
4. Press **Restart Acorn File Forge** to start the new version.
...
`/opt/acorn-file-forge/package-target` when the release workflow builds it, so
...
the downloaded package, which is kept in `~/.cache/acorn-file-forge/updates`.
