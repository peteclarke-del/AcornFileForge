...
class DesktopPathTests(unittest.TestCase):
    ...
    def test_linux_uses_xdg_directories(self) -> None:
        ...
        self.assertEqual(paths.data, Path("/home/user/.local/share/acorn-file-forge"))
        self.assertEqual(paths.config, Path("/home/user/.config/acorn-file-forge"))
        self.assertEqual(paths.cache, Path("/home/user/.cache/acorn-file-forge"))
    ...
    def test_linux_honours_an_xdg_override(self) -> None:
        ...
        self.assertEqual(paths.data, Path("/custom/data/acorn-file-forge"))

    def test_macos_uses_application_support_and_caches(self) -> None:
        paths = self._paths_for("darwin", {})
        self.assertEqual(
            paths.data, Path("/home/user/Library/Application Support/AcornFileForge")
        )
        self.assertEqual(paths.cache, Path("/home/user/Library/Caches/AcornFileForge"))
        self.assertEqual(paths.work, paths.data / "work")
    ...
    def test_windows_uses_roaming_for_state_and_local_for_rebuildable_data(self) -> None:
        ...
        self.assertEqual(paths.data, Path("C:/Users/user/AppData/Roaming/AcornFileForge"))
    ...
    def test_windows_falls_back_when_the_environment_is_bare(self) -> None:
        ...
        self.assertIn("AcornFileForge", str(paths.data))

    def test_every_platform_keeps_work_inside_a_known_directory(self) -> None:
        for platform in ("linux", "darwin", "win32"):
            with self.subTest(platform=platform):
                paths = self._paths_for(platform, {})
                self.assertIn("AcornFileForge", str(paths.work)) if platform != "linux" else None
