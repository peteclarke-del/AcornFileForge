  ...
  try {
    ...
    imageId = await page.evaluate(async () => {
      await new Promise((resolve, reject) => {
        const request = indexedDB.deleteDatabase("acorn-file-forge-private-collection-v1");
        request.onsuccess = resolve;
        request.onerror = () => reject(request.error);
      });
      const data = await window.AcornUI.api("/api/images/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ format: "ssd", title: "COLLECT" }),
      });
    ...
    try {
      await page.waitForFunction(() => document.querySelector(".pane .image-title")?.textContent.includes("blank.ssd"));
    } catch (error) {
      const state = await page.evaluate(() => ({ saved: localStorage.getItem("acorn-file-forge-dynamic-panes"), titles: [...document.querySelectorAll(".pane .image-title")].map(item => item.textContent), text: document.body.innerText.slice(0, 800) }));
    ...
    await page.locator('[name="collectionMachines"]').fill("BBC B, Electron");
    ...
    if (!await row.textContent().then(value => value.includes("Workbench SD card") && value.includes("BBC B"))) {
    ...
    const stored = await page.evaluate(async () => new Promise((resolve, reject) => {
      const request = indexedDB.open("acorn-file-forge-private-collection-v1");
      request.onsuccess = () => {
