...
from app.fat_media import build_mmfs_card, read_mmfs_card_mmb


class FatMediaTests(unittest.TestCase):
    def test_builds_a_deterministic_fat32_card_with_contiguous_beeb_mmb(self):
        with tempfile.TemporaryDirectory() as folder:
            root = Path(folder)
            source = root / "games.mmb"
            payload = bytes(range(256)) * 1200
            source.write_bytes(payload)
            card = root / "card.img"
            layout = build_mmfs_card(source, card)
            data = card.read_bytes()
            extracted = read_mmfs_card_mmb(card, layout)
        ...
        self.assertEqual(data[root_offset:root_offset + 11], b"ACORN MMFS ")
        self.assertEqual(data[root_offset + 32:root_offset + 43], b"BEEB    MMB")
