...
class ExportRouteTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.service = Mock()
        self.session = SimpleNamespace(kind="dfs", name="disk.ssd")
    ...
    def test_available_formats_are_listed_for_the_open_image(self) -> None:
        self.service.export_formats.return_value = [
            {"format": "native", "extension": "ssd", "label": "Native sector image (.ssd)"},
    ...
    def test_export_defaults_to_the_native_sector_image(self) -> None:
        output = self.root / "disk-export.ssd"
        output.write_bytes(b"SECTORS")
        self.service.export_image.return_value = (output, "disk-export.ssd")
