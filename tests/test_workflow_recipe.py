...
class WorkflowRecipeTests(unittest.TestCase):
    def test_rebuild_guide_shell_quotes_image_descriptor_and_output_names(self):
        session = SimpleNamespace(name="My Games.dat")
        recipe = {
            "sources": {"image": {
                "name": "My Base.dat",
                "size": 10,
                "sha256": "a" * 64,
                "descriptor": {
                    "name": "My Base.dsc", "size": 2, "sha256": "b" * 64,
                },
            }},
            "output": {"files": [
                {"name": "My Games.dat", "size": 10, "sha256": "c" * 64},
            ]},
        }

        readme = _workflow_readme(session, recipe, {"operations": []})

        self.assertIn("--source 'image=/path/to/My Base.dat'", readme)
        self.assertIn("--descriptor 'image=/path/to/My Base.dsc'", readme)
        self.assertIn("--output 'My Games.dat'", readme)

    @unittest.skipUnless(shutil.which("disc"), "Oaknut CLI is required")
    def test_real_dfs_workflow_replays_to_the_recorded_output(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            service = DiskService(root / "work")
            session = service.create_blank("ssd", "RECIPE")
            checkpoint = service.create_checkpoint(session, "Before import")
            payload = root / "PROGRAM"
            payload.write_bytes(b"PRINT \"REBUILT\"\r")
            service.put(session, None, "$.PROGRAM", payload, "1900", "1900", None)
    ...
    def test_bundle_contains_guarded_patch_recipe_hashes_and_rebuild_guide(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            base_path = root / "base.ssd"
            base_path.write_bytes(b"base")
            session = SimpleNamespace(
                name="games.ssd",
                kind="dfs",
                path=root / "current.ssd",
                descriptor_path=None,
                descriptor_name=None,
                target_hardware="bbc-master",
                hardware_profile={"machine": "master", "tube": False},
                compatibility_reports=[{"format": "acorn-file-forge-compatibility-report"}],
                hfe_original_path=None,
            )
            session.path.write_bytes(b"current")
            base_session = SimpleNamespace(
                kind="dfs", name="games.ssd", path=base_path, descriptor_path=None,
            )
            ...
            identities = [
                {"name": "games.ssd", "size": 4, "sha256": "a" * 64},
                {"name": WORKFLOW_PATCH_NAME, "size": 12, "sha256": "b" * 64},
            ]
            with (
                patch("app.workflow_recipe._checkpoint_source", return_value=(base_path, None, {"name": "games.ssd", "targetHardware": "bbc-master"})),
                ...
                patch("app.workflow_recipe._save_replay_outputs", return_value=[{"name": "games.ssd", "size": 7, "sha256": "c" * 64}]),
            ...
            with zipfile.ZipFile(output) as archive:
                ...
                self.assertEqual(recipe["actions"], [{
                    "action": "apply-patch",
                    "source": "changes",
                    "targetHardware": "bbc-master",
                }])
                self.assertEqual(recipe["output"]["files"][0]["sha256"], "c" * 64)
                self.assertEqual(recipe["decisions"]["hardwareProfile"]["machine"], "master")
