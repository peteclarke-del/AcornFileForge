...
class ScpTests(unittest.TestCase):
    ...
    def test_open_scp_decodes_clean_round_tripping_capture(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            with (
                ...
                patch.object(service, "identify_kind", return_value="dfs"),
            ):
                working, kind, original_path, read_only, warnings = service._open_scp(original)
            self.assertEqual(kind, "dfs")
    ...
    def test_open_scp_rejects_non_acorn_filesystem(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            original = Path(folder) / "capture.scp"
            original.write_bytes(b"SCP" + bytes(100))
            service = DiskService(Path(folder) / "work")
            ...
            with (
                ...
                with self.assertRaisesRegex(DiskError, "cannot be browsed as an Acorn disk image"):
    ...
    def test_scp_decode_restores_one_omitted_adfs_l_tail_sector(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            original = Path(folder) / "capture.scp"
            original.write_bytes(b"SCP" + bytes(100))
            service = DiskService(Path(folder) / "work")
            ...
            def convert(arguments):
                if any(argument.startswith("-conv:SCP_FLUX_STREAM") for argument in arguments):
                    ...
                    _write_output(arguments, bytes(655_360 - 256))
                return "Invalid rpm or tracklen"

            with (
                patch.object(service, "_run_hxcfe", side_effect=convert),
                patch.object(service, "_run", return_value=""),
                patch.object(service, "identify_kind", return_value="adfs"),
            ):
                working, kind, _original, _read_only, warnings = service._open_scp(original)
            self.assertEqual(kind, "adfs")
            self.assertEqual(working.suffix, ".adl")
            self.assertEqual(working.stat().st_size, 655_360)
    ...
    def test_scp_decode_rejects_a_broken_directory_tree(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            def convert(arguments):
                _write_output(arguments, bytes(655_360))
                return ""
    ...
    def test_export_formats_offer_flux_containers_for_dfs_and_adfs_s_m_l(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            dfs_image = Path(folder) / "disk.ssd"
            dfs_image.write_bytes(bytes(204_800))
            dfs_session = ImageSession("a" * 32, "disk.ssd", "dfs", dfs_image)
            formats = {entry["format"] for entry in service.export_formats(dfs_session)}
            self.assertEqual(formats, {"native", "hfe", "scp"})

            adfs_l = Path(folder) / "disk.adl"
            adfs_l.write_bytes(bytes(655_360))
            adfs_session = ImageSession("b" * 32, "disk.adl", "adfs", adfs_l)
            self.assertEqual(
                {entry["format"] for entry in service.export_formats(adfs_session)},
                {"native", "hfe", "scp"},
            )

            adfs_d = Path(folder) / "disk.adf"
            adfs_d.write_bytes(bytes(800 * 1024))
            adfs_d_session = ImageSession("c" * 32, "disk.adf", "adfs", adfs_d)
            self.assertEqual(
                {entry["format"] for entry in service.export_formats(adfs_d_session)},
                {"native"},
            )

    def test_export_formats_empty_for_unsupported_kinds(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            image = Path(folder) / "blank.mmb"
            image.write_bytes(bytes(10))
            session = ImageSession("a" * 32, "blank.mmb", "mmb", image)
            self.assertEqual(service.export_formats(session), [])

    def test_export_native_copies_current_sectors_with_canonical_extension(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            image = Path(folder) / "disk.ssd"
            image.write_bytes(b"payload" + bytes(204_800 - 7))
            session = ImageSession("a" * 32, "disk.ssd", "dfs", image)
            output, name = service.export_image(session, "native")
            self.assertEqual(output.read_bytes(), image.read_bytes())
            self.assertTrue(name.endswith(".ssd"))

    def test_export_scp_verifies_round_trip_before_returning(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            image = Path(folder) / "disk.ssd"
            image.write_bytes(bytes(204_800))
            session = ImageSession("a" * 32, "disk.ssd", "dfs", image)
    ...
    def test_export_rejects_unavailable_format(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            session = ImageSession("a" * 32, "disk.adf", "adfs", image)
...
class ScpSaveTests(unittest.TestCase):
    ...
    def test_unedited_session_downloads_the_untouched_original_capture(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            ...
            session = self._session(
                folder, suffix=".ssd", size=204_800, kind="dfs", dirty=False
            )
    ...
    def test_edited_session_is_re_encoded_and_verified(self) -> None:
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            session = self._session(
                folder, suffix=".ssd", size=204_800, kind="dfs", dirty=True
            )
    ...
    def test_edited_adfs_l_save_survives_an_omitted_tail_sector(self) -> None:
        """Regression: the SCP save path once lacked the tail-sector repair.

        HxCFE can drop the blank final sector when decoding its own ADFS-L
        output. Without the repair the verification compared 655,104 bytes
        against 655,360 and refused a save that was in fact byte-exact.
        """
        with tempfile.TemporaryDirectory() as folder:
            service = DiskService(Path(folder) / "work")
            session = self._session(
                folder, suffix=".adl", size=655_360, kind="adfs", dirty=True
            )
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            def convert(arguments):
                if any(item.startswith("-conv:SCP_FLUX_STREAM") for item in arguments):
                    ...
                    _write_output(arguments, b"\xFF" * 204_800)
                return ""
...
class ScpSessionPersistenceTests(unittest.TestCase):
    ...
    def _persisted_session(self, work: Path, *, read_only: bool) -> str:
        ...
        working = folder / "capture.ssd"
        working.write_bytes(bytes(204_800))
    ...
    def _prepared(self, folder: str, container: str, kind: str, size: int, suffix: str):
        ...
        def convert(arguments):
            if any(item.startswith(f"-conv:{plugin}") for item in arguments):
                ...
                _write_output(arguments, bytes(size - 256))
            return ""
            ...
            with self.subTest(container=container):
                with tempfile.TemporaryDirectory() as folder:
                    service, session, convert = self._prepared(
                        folder, container, "adfs", 655_360, ".adl"
                    )
                    with patch.object(service, "_run_hxcfe", side_effect=convert):
    ...
    def test_both_containers_pass_the_original_as_the_timing_reference(self) -> None:
        for container in ("hfe", "scp"):
            with self.subTest(container=container):
                with tempfile.TemporaryDirectory() as folder:
                    service, session, convert = self._prepared(
                        folder, container, "dfs", 204_800, ".ssd"
                    )
                    seen: list[list[str]] = []
