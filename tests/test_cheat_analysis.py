...
class CheatAnalysisTests(unittest.TestCase):
    ...
    def test_disassembly_diagnoses_runtime_payload(self):
        rows = [{"address": 0x2000 + index, "mnemonic": "EQUB", "operand": "&00", "reachable": index == 0}
    ...
    def test_report_is_explicitly_read_only(self):
        report = cheat_report(path="$.GAME", kind="basic", findings=[], title="Frak", machine="BBC B")
        self.assertTrue(report["readOnly"])
        self.assertEqual(len(report["referenceSearches"]), 3)
