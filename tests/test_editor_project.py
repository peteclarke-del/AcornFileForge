...
class EditorProjectTests(unittest.TestCase):
    ...
    def test_annotations_follow_file_and_directory_moves(self):
        ...
        self.assertIn("-|-|$.Arcade.Frak.!BOOT", session.editor_projects)
        self.assertNotIn("-|-|$.Games.Frak.!BOOT", session.editor_projects)
    ...
    def test_annotations_are_removed_with_deleted_directory(self):
        ...
        session = SimpleNamespace(
            lock=RLock(),
            editor_projects={
                "4|0|$.Games.Frak.!BOOT": {"notes": "loader"},
                "4|1|$.Games.Frak.!BOOT": {"notes": "other side"},
            },
        )
        removed = service.delete_editor_projects(session, ["$.Games"], 4, 0)
        self.assertEqual(removed, 1)
        self.assertEqual(list(session.editor_projects), ["4|1|$.Games.Frak.!BOOT"])
