...
and L use 256-byte sectors and ADFS D, E and F use 1024. The kernel geometry
must already match the disk, normally through ``setfdprm`` or a device node such
as ``/dev/fd0u800``. This module therefore verifies what it read against a known
Acorn geometry and refuses anything that does not match, instead of handing back
a plausible-looking image of the wrong shape.
...
class FloppyGeometry:
    """One Acorn floppy layout, named as the workbench names its images."""
...
ACORN_GEOMETRIES: dict[str, FloppyGeometry] = {
    geometry.identifier: geometry
    for geometry in (
        FloppyGeometry("dfs-40", "DFS 40 track, single sided", ".ssd", 40, 1, 10, 256),
        FloppyGeometry("dfs-80", "DFS 80 track, single sided", ".ssd", 80, 1, 10, 256),
        FloppyGeometry("dfs-80-ds", "DFS 80 track, double sided", ".dsd", 80, 2, 10, 256),
        FloppyGeometry("adfs-s", "ADFS S, 160 KiB", ".ads", 40, 1, 16, 256),
        FloppyGeometry("adfs-m", "ADFS M, 320 KiB", ".adm", 80, 1, 16, 256),
        FloppyGeometry("adfs-l", "ADFS L, 640 KiB", ".adl", 80, 2, 16, 256),
        FloppyGeometry("adfs-d", "ADFS D, 800 KiB", ".adf", 80, 2, 5, 1024),
        FloppyGeometry("adfs-e", "ADFS E, 800 KiB", ".adf", 80, 2, 5, 1024),
    )
}
...
def geometry(identifier: str) -> FloppyGeometry:
    ...
    found = ACORN_GEOMETRIES.get(str(identifier or "").strip().lower())
    if found is None:
        raise FloppyError(
            "Choose a floppy geometry: " + ", ".join(sorted(ACORN_GEOMETRIES)) + "."
        )
    return found
...
def geometry_for_size(size: int) -> FloppyGeometry | None:
    ...
    matches = {item.identifier: item for item in ACORN_GEOMETRIES.values() if item.size == size}
    if len(matches) != 1:
        return None
...
class FloppyDevice:
    ...
    def write(
        ...
        *,
        ...
        if layout is None:
            raise FloppyError(
                f"{path.name} is {size:,} bytes, which is not one of the Acorn floppy "
