...
release=${ACORN_RPM_RELEASE:-1}

if ! printf '%s\n' "$release" | grep -Eq '^[0-9][A-Za-z0-9._]*$'; then
    echo "ACORN_RPM_RELEASE must start with a digit, for example 1 or 2.fc41." >&2
    exit 2
fi
...
payload="$build_root/acorn-file-forge-$rpm_version"
mkdir -p "$payload"
...
tarball="$build_root/rpmbuild/SOURCES/acorn-file-forge-$rpm_version.tar.gz"
mkdir -p \
...
tar --sort=name --mtime="@$SOURCE_DATE_EPOCH" --owner=0 --group=0 --numeric-owner \
    -czf "$tarball" -C "$build_root" "acorn-file-forge-$rpm_version"

spec="$build_root/rpmbuild/SPECS/acorn-file-forge.spec"
sed \
    -e "s/@VERSION@/$rpm_version/g" \
    -e "s/@RELEASE@/$release/g" \
    "$project_root/packaging/rpm/acorn-file-forge.spec.in" > "$spec"
